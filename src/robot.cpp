...
void Robot::launchMission() {
    ...
    if (mode == 2) {
        Logger::info("Launchin route mission.");
        setNextGoal(true);
    }
}
    ...
    if (_first) {
        if (mission.points.size() > 0) {
            ...
            if (!getTargetPose(mission.points.front(), target_pose)) {
                Logger::warn("Failed to set first goal : could not get target pose.");
                return false;
            }
            return startMoveBaseGoal(target_pose);
        }
        else {
            Logger::warn("Failed to set first goal : 0 mission points.");
            resetMission();
            return false;
        }
    }
        ...
        if (mission.points.size() > 1) {
            ...
            if (!getTargetPose(*(mission.points.erase(mission.points.begin())), target_pose)) {
                Logger::warn("Failed to set next goal : could not get target pose");
                return false;
            }
            return startMoveBaseGoal(target_pose);
        }
...
void Robot::updateUtmTransform() {
    //Convert GPS position to geometric data
    geometry_msgs::PointStamped geometric_pos;
    latLongToUtm(position, geometric_pos);
    tf::Vector3 transform_origin = tf::Vector3(geometric_pos.point.x, geometric_pos.point.y, geometric_pos.point.z);

    //Prepare orientation data
    tf::Quaternion q;
    q.setRPY(0, 0, yaw);

    //Create the UTM -> baselink transform, and inverse it to get baselink -> UTM
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "utm", "base_link"));
}