...
void MiradorDriver::missionCallback(const mirador_driver::Mission& _mission) {
    if (robot.position.latitude != 0 && robot.position.longitude != 0) {
        if (_mission.type == 1 && (robot.mode == 0 || robot.mode == 1)) //guide missions can replace guide missions
        {
        ...
        else if (robot.mode == 0) //if no active mission
        {
            switch (_mission.type) {
            case 2: //route mission
                Logger::info("Route mission received");
                robot.mode = _mission.type;
                robot.mission.id = _mission.id;
                robot.mission.points = _mission.points;
                break;
            case 3: //exploration mission
...
void MiradorDriver::sendFakeImuMessage() {
    sensor_msgs::NavSatFix msg;
    msg.latitude = 48.711242;
    msg.longitude = 2.217901;
    navSatFixCallback(msg);
}

void MiradorDriver::fakePing() {
